...
import { formatMiles, formatNumber } from '@/lib/miles-goal/format'
import styles from './MilesAchievementFormula.module.css'

type FormulaItem = { amount: number; label: string }
...
export default function MilesAchievementFormula({ candidate, card, displays, currentUsableMiles, rewardCurrency }: {
  ...
  const items: FormulaItem[] = [
    ...
    ...(baseMiles > 0 ? [{ amount: baseMiles, label: `Everyday spending over ${candidate.months_to_goal} ${candidate.months_to_goal === 1 ? 'month' : 'months'}` }] : []),
    ...bonusItems,
  ]
  ...
  return <div className={styles.formula} aria-label={`Miles calculation reaching your goal in ${candidate.months_to_goal} months`}>
    <div className={styles.scroller}><div className={styles.equation}>
      {items.map((item, index) => <Fragment key={`${item.label}-${index}`}>{index > 0 && <span className={styles.operator} aria-hidden="true">+</span>}<div className={styles.item}><strong>{formatNumber(item.amount)}</strong><span>{item.label}</span></div></Fragment>)}
