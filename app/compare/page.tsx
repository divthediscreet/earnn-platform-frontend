...
function ComparisonModal({ cards, catalogueRewardRates, details, onClose, onRemove, showOptionalCategories, onToggleOptionalSection }: {
  ...
  return <div role="dialog" aria-modal="true" aria-label="Compare selected cards" onClick={onClose} style={{ position: 'fixed', inset: 0, zIndex: 300, padding: 'clamp(12px, 3vw, 32px)', background: 'rgba(5,18,43,.72)', backdropFilter: 'blur(7px)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
    <div onClick={event => event.stopPropagation()} style={{ width: 'min(1280px, 100%)', maxHeight: '94vh', overflow: 'auto', borderRadius: 26, background: '#F3F6FC', boxShadow: '0 32px 96px rgba(3,12,30,.48)', border: '1px solid rgba(255,255,255,.35)' }}>
      <div style={{ position: 'sticky', top: 0, zIndex: 3, padding: '25px clamp(20px, 3vw, 34px)', color: 'white', display: 'flex', justifyContent: 'center', alignItems: 'center', overflow: 'hidden', background: 'radial-gradient(circle at 82% 0%, rgba(52,112,222,.65), transparent 32%), linear-gradient(118deg, #071D4A 0%, #0E3785 58%, #123F91 100%)' }}>
        ...
        <div style={{ position: 'relative', zIndex: 1, textAlign: 'center' }}><div style={{ display: 'inline-flex', alignItems: 'center', padding: '5px 9px', borderRadius: 100, background: 'rgba(255,255,255,.1)', color: '#E7C65A', fontSize: 10.5, letterSpacing: '.11em', fontWeight: 900, textTransform: 'uppercase' }}>Earnn card compare</div><div style={{ marginTop: 10, fontSize: 'clamp(22px, 3vw, 30px)', fontWeight: 900, letterSpacing: '-.035em' }}>Your cards, side by side.</div><div style={{ marginTop: 5, color: '#C7D7F6', fontSize: 12.5 }}>Compare rewards, value and benefits at a glance.</div></div>
        <button onClick={onClose} aria-label="Close comparison" style={{ position: 'absolute', zIndex: 2, right: 'clamp(20px, 3vw, 34px)', top: '50%', transform: 'translateY(-50%)', width: 34, height: 34, borderRadius: 8, border: '1px solid rgba(255,255,255,.25)', background: 'rgba(255,255,255,.1)', color: 'white', cursor: 'pointer', fontSize: 20 }}>×</button>
      </div>
